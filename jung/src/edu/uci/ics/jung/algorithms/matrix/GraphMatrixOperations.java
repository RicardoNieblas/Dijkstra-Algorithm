...
import edu.cetys.cinap.icc.algorithms.exceptions.GraphInstanciationException;
...
public class GraphMatrixOperations
{
    ...
    public static <V,E> SparseDoubleMatrix2D graphToMatrix(Graph<V,E> graph, Map<E,Number> nev) throws GraphInstanciationException, GraphTypeException
    {
    	if (nev == null)
    		throw new GraphInstanciationException("Instationation Exception: Adyacency data must be provided");
    
    	if(!( graph instanceof DirectedSparseMultigraph))
			throw new GraphTypeException("Type Error: Only directed graphs accepted");
    	...
    	for(E e: nev.keySet()){
    		 ...
    		 int i = ((Number) pair.getFirst()).intValue();
    		 int j = ((Number) pair.getSecond()).intValue();
    		 matrix.set(i, j, nev.get(e).doubleValue());
    	}
